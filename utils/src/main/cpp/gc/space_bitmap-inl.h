...
inline bool SpaceBitmap<kAlignment>::AtomicTestAndSet(const void* obj) {
...
template<size_t kAlignment>
inline bool SpaceBitmap<kAlignment>::Test(const void* obj) const {
  uintptr_t addr = reinterpret_cast<uintptr_t>(obj);
...
inline void SpaceBitmap<kAlignment>::VisitMarkedRange(uintptr_t visit_begin,
  ...
  for (uintptr_t i = visit_begin; i < visit_end; i += kAlignment) {
    void* obj = reinterpret_cast<mirror::Object*>(i);
    if (Test(obj)) {
      visitor(obj);
    }
  }
  ...
  if (index_start < index_end) {
    ...
    if (left_edge != 0) {
      ...
      do {
        ...
        void* obj = reinterpret_cast<void*>(ptr_base + shift * kAlignment);
        visitor(obj);
...
inline bool SpaceBitmap<kAlignment>::Modify(const void* obj) {
  uintptr_t addr = reinterpret_cast<uintptr_t>(obj);
  DCHECK_GE(addr, heap_begin_);
  DCHECK(HasAddress(obj)) << obj;
  const uintptr_t offset = addr - heap_begin_;
