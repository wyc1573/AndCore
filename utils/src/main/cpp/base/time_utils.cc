...
std::string FormatDuration(uint64_t nano_duration, TimeUnit time_unit,
  ...
  if (fractional_part == 0) {
    return android::base::StringPrintf("%" PRIu64 "%s", whole_part, unit);
  } else {
    ...
    return android::base::StringPrintf("%" PRIu64 ".%s%s", whole_part, fraction_buffer, unit);
  }
}
...
#endif
  return android::base::StringPrintf("%04d-%02d-%02d %02d:%02d:%02d",
